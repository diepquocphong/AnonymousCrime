...
namespace KINEMATION.Shared.FbxExporter.Editor
{
    public static class AsciiFbxExporter
    {
        ...
        private const int FbxCustomTimeMode = 14;
        ...
        public static bool Export(ExportOptions options, out string error)
        {
            ...
            try
            {
                ...
                if (stripRootNode)
                {
                    for (int i = 0; i < exportInstance.transform.childCount; i++)
                    {
                        CollectExportNodes(exportInstance.transform.GetChild(i), exportInstance.transform, -1,
                            exportNodes, skeletonTransforms);
                    }
                }
                else
                {
                ...
                ExportDocument document = BuildDocument(options, rootTrack, exportNodes, fbxSampleTimes, sampleRate);
        ...
        private static ExportDocument BuildDocument(ExportOptions options, ExportRootTrack rootTrack,
            List<ExportNode> exportNodes, List<long> sampleTimes, float resolvedFrameRate)
        {
            ...
            out string error)
        {
            ...
            for (int sampleIndex = 0; sampleIndex < sampleCount; sampleIndex++)
            {
                ...
                if (rootTrack != null)
                {
                    AddRootTrackSample(rootTrack, rootPosition, rootRotation, rootScale, sampleIndex);
                }
                ...
                for (int nodeIndex = 0; nodeIndex < exportNodes.Count; nodeIndex++)
                {
                    ...
                    if (transform == exportInstance.transform && rootMotionCurves != null && rootMotionCurves.HasAny)
                    {
            ...
            Vector3 scale, int sampleIndex)
        {
            ...
            rootTrack.translationX.Add(position.x);
            rootTrack.translationY.Add(position.y);
            rootTrack.translationZ.Add(position.z);

            rootTrack.rotationX.Add(rawEuler.x);
            rootTrack.rotationY.Add(rawEuler.y);
            rootTrack.rotationZ.Add(rawEuler.z);

            rootTrack.scaleX.Add(scale.x);
            rootTrack.scaleY.Add(scale.y);
            rootTrack.scaleZ.Add(scale.z);
        }
        ...
        private static void AppendGlobalSettings(StringBuilder builder, long stopTime, float frameRate)
        {
        ...
        private static void AppendNodeAttribute(StringBuilder builder, ExportNode node, bool isRoot)
        {
        ...
        private static void AppendConnections(StringBuilder builder, ExportDocument document)
        {
            ...
            for (int i = 0; i < document.nodes.Count; i++)
            {
                ...
                long parentModelId = node.parentIndex >= 0
                    ...
                    : (node.isSkeletonNode || document.rootTrack == null ? 0 : document.sceneRootModelId);
