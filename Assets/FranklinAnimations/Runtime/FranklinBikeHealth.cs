...
namespace FranklinGame.Vehicles
{
    ...
    public sealed class FranklinBikeHealth : MonoBehaviour
    {
        ...
        [Tooltip("Only accepted heavy impacts damage the seated Player.")]
        ...
        private void OnImpactAccepted(bool isHeavy, float severity)
        {
            ...
            // Resolve the rider before changing the Attribute. EventDestroyed is
            // synchronous and terminal destruction must capture the seated rider
            // before the heavy-impact ragdoll callback releases them.
            Character rider = m_BikeEntry != null
                ? m_BikeEntry.SeatedCharacter
                : null;
            ...
            if (isHeavy && m_ApplySeatedPlayerDamage && rider != null &&
                rider.Player != null)
            {
                ApplyDamageToPlayer(
                    rider,
