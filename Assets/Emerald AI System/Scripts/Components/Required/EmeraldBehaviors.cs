...
namespace EmeraldAI
{
    ...
    public class EmeraldBehaviors : MonoBehaviour
    {
        ...
        /// <summary>
        /// Continuously updates the BehaviorObject. This acts like an Update function that can run within this behavior using the information from the passed EmeraldComponent and its 
        /// </summary>
        public virtual void BehaviorUpdate()
        {
            if (EmeraldComponent.AnimationComponent.IsDead || !EmeraldComponent.m_NavMeshAgent.enabled)
                return;
        ...
        {
            if (EmeraldComponent.CombatComponent.CombatState && EmeraldComponent.MovementComponent.AIAgentActive && EmeraldComponent.CombatTarget && EmeraldComponent.m_NavMeshAgent.enabled)
            {
        ...
        public virtual void OnDetectTarget()
        {
            ...
            if (isActiveAndEnabled && EmeraldComponent.m_NavMeshAgent.enabled) EmeraldComponent.m_NavMeshAgent.SetDestination(transform.position);
        }
