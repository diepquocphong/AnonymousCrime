...
namespace EmeraldAI
{
    ...
    public class EmeraldSounds : MonoBehaviour
    {
        ...
        /// <summary>
        /// Plays a random jump sound when an AI jumps using a NavMesh LInk Component.
        /// </summary>
        public void PlayJumpSound()
        {
            if (SoundProfile.JumpSounds.Count > 0)
            {
                if (!m_AudioSource.isPlaying)
                {
                    m_AudioSource.Stop();
                    m_AudioSource.volume = SoundProfile.JumpVolume;
                    m_AudioSource.PlayOneShot(SoundProfile.JumpSounds[Random.Range(0, SoundProfile.JumpSounds.Count)]);
                }
                else if (!m_SecondaryAudioSource.isPlaying)
                {
                    m_SecondaryAudioSource.Stop();
                    m_SecondaryAudioSource.volume = SoundProfile.JumpVolume;
                    m_SecondaryAudioSource.PlayOneShot(SoundProfile.JumpSounds[Random.Range(0, SoundProfile.JumpSounds.Count)]);
                }
                else
                {
                    m_EventAudioSource.Stop();
                    m_EventAudioSource.volume = SoundProfile.JumpVolume;
                    m_EventAudioSource.PlayOneShot(SoundProfile.JumpSounds[Random.Range(0, SoundProfile.JumpSounds.Count)]);
                }
            }
        }

        /// <summary>
        /// Plays a random land sound when an AI lands using a NavMesh LInk Component.
        /// </summary>
        public void PlayLandSound()
        {
            if (SoundProfile.LandSounds.Count > 0)
            {
                if (!m_AudioSource.isPlaying)
                {
                    m_AudioSource.Stop();
                    m_AudioSource.volume = SoundProfile.LandVolume;
                    m_AudioSource.PlayOneShot(SoundProfile.LandSounds[Random.Range(0, SoundProfile.LandSounds.Count)]);
                }
                else if (!m_SecondaryAudioSource.isPlaying)
                {
                    m_SecondaryAudioSource.Stop();
                    m_SecondaryAudioSource.volume = SoundProfile.LandVolume;
                    m_SecondaryAudioSource.PlayOneShot(SoundProfile.LandSounds[Random.Range(0, SoundProfile.LandSounds.Count)]);
                }
                else
                {
                    m_EventAudioSource.Stop();
                    m_EventAudioSource.volume = SoundProfile.LandVolume;
                    m_EventAudioSource.PlayOneShot(SoundProfile.LandSounds[Random.Range(0, SoundProfile.LandSounds.Count)]);
                }
            }
        }
        ...
        public void Footstep()
        {
            ...
            if (EmeraldComponent.MovementComponent.CanPlayWalkFootstepSound() || EmeraldComponent.MovementComponent.CanPlayRunFootstepSound())
            {
                ...
                if (SoundProfile.FootStepSounds.Count > 0 && !EmeraldComponent.m_NavMeshAgent.isOnOffMeshLink)
                {
                    if (!m_AudioSource.isPlaying)
                    {
        ...
        public void WalkFootstepSound()
        {
            ...
            if (EmeraldComponent.MovementComponent.CanPlayWalkFootstepSound() && !EmeraldComponent.m_NavMeshAgent.isOnOffMeshLink)
            {
        ...
        public void RunFootstepSound()
        {
            ...
            if (EmeraldComponent.MovementComponent.CanPlayRunFootstepSound() && !EmeraldComponent.m_NavMeshAgent.isOnOffMeshLink)
            {
                if (SoundProfile.FootStepSounds.Count > 0)
                {
                    if (!m_AudioSource.isPlaying)
