...
namespace EmeraldAI
{
    ...
    public class EmeraldCombat : MonoBehaviour, ICombat
    {
        ...
        public delegate void StartSlowedHandler();
        public event StartSlowedHandler OnStartSlowed;
        public delegate void EndSlowedHandler();
        public event StartSlowedHandler OnEndSlowed;
        ...
        public Coroutine SlowedCoroutine;
        #endregion
        ...
        /// <summary>
        /// Sets the slowed state. If the value is less than 1, the AI is slowed, if the value is 1, the AI's slowed state will be stoped and its speed will return to normal.
        /// <br/><br/>Note: You need to pass a value between 0 (fully slowed) and 1 (not slowed);
        /// </summary>
        public void SetSlowedState (float SlowPercentage)
        {
            bool slowed = SlowPercentage < 1f;

            (slowed ? OnStartSlowed : OnEndSlowed)?.Invoke();

            EmeraldComponent.AnimationComponent.IsSlowed = slowed;
            EmeraldComponent.AIAnimator.speed = slowed ? SlowPercentage : 1f;
            if (EmeraldComponent.MovementComponent.MovementType == EmeraldMovement.MovementTypes.NavMeshDriven) EmeraldComponent.m_NavMeshAgent.speed = slowed ? EmeraldComponent.m_NavMeshAgent.speed * SlowPercentage : EmeraldComponent.MovementComponent.RunSpeed;
        }

        /// <summary>
        /// Called when an ability generates a stun (used through the ICombat interface).
        /// </summary>
