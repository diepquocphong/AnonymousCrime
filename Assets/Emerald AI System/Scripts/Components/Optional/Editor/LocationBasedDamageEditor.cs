...
namespace EmeraldAI.Utility
{
    ...
    public class LocationBasedDamageEditor : Editor
    {
        ...
        SerializedProperty HideSettingsFoldout, LBDSettingsFoldout, LBDComponentsTag, SetCollidersLayerAndTag, GetColliderSource, CustomTransformSource;
        ...
        private void OnEnable()
        {
            ...
            GetColliderSource = serializedObject.FindProperty("GetColliderSource");
            CustomTransformSource = serializedObject.FindProperty("CustomTransformSource");
            InitializeList();
            InitializeLayers();
        }
        ...
        void LBDSettings (LocationBasedDamage self)
        {
            ...
            if (LBDSettingsFoldout.boolValue)
            {
                ...
                CustomEditorProperties.TextTitleWithDescription("Location Based Damage Settings", "The Location Based Damage component allows each collider to detect damage and apply a customizable damage multiplier based on the damage received. The hit effect that will play upon impact, " +
                ...
                CustomEditorProperties.CustomPropertyField(SetCollidersLayerAndTag, "Set Colliders Layer and Tag", "Controls whether or not the Location Based Damage component will set the detected collider's tag and layer. Users can customize the tag and layer by enabling this setting.", true);
                ...
                CustomEditorProperties.CustomPropertyField(GetColliderSource, "Get Collider Source", "Controls the source that the colliders will come from. By default, this is This Transform, however, Custom Source can be used if needed.", true);
                if (self.GetColliderSource == LocationBasedDamage.GetColliderSources.CustomTransfrom)
                {
                    GUILayout.Space(-5);
                    if (self.CustomTransformSource == null)
                    {
                        CustomEditorProperties.DisplayWarningMessage("Custom Transform Source cannot be left blank. Please apply a Custom Transform Source and press the Get Colliders button.");
                    }
                    CustomEditorProperties.BeginIndent();
                    CustomEditorProperties.CustomPropertyField(CustomTransformSource, "Custom Transform Source", "Allows a custom transform source to be used to get the colliders. This is needed for systems that change the bone structure of animated models (such as Puppet Master).", true);
                    CustomEditorProperties.EndIndent();
                }

                if (GUILayout.Button("Get Colliders"))
                {
                    //Added custom transform source option
                    if (self.GetColliderSource == LocationBasedDamage.GetColliderSources.CustomTransfrom)
                    {
                        var m_Colliders = self.CustomTransformSource.GetComponentsInChildren<Collider>();
