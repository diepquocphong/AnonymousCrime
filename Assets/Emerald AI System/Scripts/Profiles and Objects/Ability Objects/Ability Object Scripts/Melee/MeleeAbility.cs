...
namespace EmeraldAI
{
    ...
    public class MeleeAbility : EmeraldAbilityObject
    {
        ...
        public AbilityData.SlowedData SlowedSettings;
        ...
        public override void InvokeAbility(GameObject Owner, Transform AttackTransform = null)
        {
            ...
            if (SlowedSettings.Enabled && SlowedSettings.RollForSlowEffect())
            {
                EmeraldSystem TargetEmeraldComponent =  m_ICombat.TargetTransform().GetComponent<EmeraldSystem>();

                //The slow feature only works for AI
                if (m_ICombat != null && TargetEmeraldComponent)
                {
                    if (!TargetEmeraldComponent.AnimationComponent.IsSlowed || TargetEmeraldComponent.AnimationComponent.IsSlowed && TargetEmeraldComponent.AIAnimator.speed >= SlowedSettings.SlowPercentage * 0.01f)
                    {
                        ...
                        TargetEmeraldComponent.CombatComponent.SlowedCoroutine = MonoBehaviourRef.StartCoroutine(SlowedSettings.SlowEffectSequence(m_ICombat.TargetTransform(), m_ICombat));
                    }
                }
            }
