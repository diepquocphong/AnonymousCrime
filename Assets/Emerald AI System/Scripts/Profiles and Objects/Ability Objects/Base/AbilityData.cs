    ...
    public class AbilityData
    {
        ...
        */
        ...
        public class KnockbackData
        {
            ...
            [Range(1f, 8f)]
            [Tooltip("Controls the how far back the target will be knocked back.")]
        ...
        [System.Serializable]
        public class SlowedData
        {
            [HideInInspector] public bool Enabled = false;
            [Range(0, 100)]
            [Tooltip("Controls the odds for triggering a slow effect when this ability hits an enemy target.")]
            public float OddsToSlow = 50;
            [Range(0, 100)]
            [Tooltip("Controls the percentage the target will be slowed. Example: A value of 25% means the target is slowed by 25% and will be moving at 75% of its normal speed.")]
            public int SlowPercentage = 25;
            public bool CanBeBlocked = false;
            [Range(1, 15)]
            [Tooltip("Controls the duration (in seconds) of how long a target will be slowed.")]
            public float SlowDuration = 4;

            public IEnumerator SlowEffectSequence(Transform Target, ICombat TargetICombat)
            {
                EmeraldSystem TargetEmeraldComponent = Target.GetComponent<EmeraldSystem>();

                //If the target is blocking, cancel the slow effect
                if (TargetICombat.IsBlocking() && CanBeBlocked || TargetICombat.IsDodging())
                {
                    yield break;
                }
                ...
                TargetEmeraldComponent.CombatComponent.SetSlowedState(1f - (SlowPercentage * 0.01f));

                while (t < 1f)
                {
                    t = elapsed / SlowDuration;
                    ...
                    if (TargetEmeraldComponent && TargetEmeraldComponent.AnimationComponent.IsDead)
                    {
                        TargetEmeraldComponent.CombatComponent.SetSlowedState(1f);
                        yield break;
                    }
            ...
            /// <summary>
            /// Rolls for a Slow Effect (using the odds through an ability) and returns true if successful.
            /// </summary>
            public bool RollForSlowEffect()
            {
                int Roll = Random.Range(1, 101);
                return (Roll <= OddsToSlow);
            }
        }
