...
namespace EmeraldAI.Utility
{
    ...
    public class EmeraldSoundProfileEditor : Editor
    {
        ...
        SerializedProperty WalkFootstepVolumeProp, RunFootstepVolumeProp, BlockVolumeProp, AttackVolumeProp, DeathVolumeProp, IdleVolumeProp, InjuredVolumeProp, InjuredSoundOddsProp,
            EquipVolumeProp, UnequipVolumeProp, WarningVolumeProp, RangedEquipVolumeProp, RangedUnequipVolumeProp, JumpVolumeProp, LandVolumeProp;

        //Objects
        SerializedProperty SheatheWeaponProp, UnsheatheWeaponProp, RangedSheatheWeaponProp, RangedUnsheatheWeaponProp;

        //Bools
        SerializedProperty IdleSoundsFoldout, FootstepSoundsFoldout, InteractSoundsFoldout, EquipAndUnequipSoundsFoldout, AttackSoundsFoldout, InjuredSoundsFoldout, BlockSoundsFoldout, WarningSoundsFoldout, DeathSoundsFoldout, JumpLandSoundsFoldout;

        //List
        SerializedProperty AttackSoundsProp, InjuredSoundsProp, WarningSoundsProp, DeathSoundsProp, FootStepSoundsProp, IdleSoundsProp, BlockingSoundsProp, JumpSoundsProp, LandSoundsProp;
        ...
        void InitializeProperties()
        {
            ...
            JumpVolumeProp = serializedObject.FindProperty("JumpVolume");
            LandVolumeProp = serializedObject.FindProperty("LandVolume");
            ...
            JumpLandSoundsFoldout = serializedObject.FindProperty("JumpLandSoundsFoldout");
            ...
            JumpSoundsProp = serializedObject.FindProperty("JumpSounds");
            LandSoundsProp = serializedObject.FindProperty("LandSounds");
        ...
        void WarningSounds(EmeraldSoundProfile self)
        {
            ...
            if (WarningSoundsFoldout.boolValue)
            {
                ...
                CustomEditorProperties.TextTitleWithDescription("Warning Sounds", "Controls the sounds that play when an AI receives damage.", false);

                CustomEditorProperties.ImportantTutorialButton("Note: Warning Sounds are only used by Cautious Behavior Types with a Confidence greater than Coward and are done through Animation Events. For a tutorial on this, " +
        ...
        void JumpLandSounds(EmeraldSoundProfile self)
        {
            JumpLandSoundsFoldout.boolValue = EditorGUILayout.Foldout(JumpLandSoundsFoldout.boolValue, "Jump & Land Sounds", true, FoldoutStyle);

            if (JumpLandSoundsFoldout.boolValue)
            {
                CustomEditorProperties.BeginFoldoutWindowBox();

                CustomEditorProperties.TextTitleWithDescription("Jump & Land Sounds", "Controls the sounds that play when an AI jumps or lands using a NavMesh Link Component.", false);

                CustomEditorProperties.ImportantTutorialButton("Note: AI will need to have an Emerald NavMesh Link component attached in order for them to use the NavMesh Link sounds.", "https://black-horizon-studios.gitbook.io/emerald-ai-wiki/emerald-components-optional/navmesh-link-component");

                CustomEditorProperties.CustomFloatSlider(new Rect(), new GUIContent(), JumpVolumeProp, "Jump Volume", 0, 1);
                CustomEditorProperties.CustomHelpLabelField("Controls the volume of Jump Sounds.", true);

                CustomEditorProperties.CustomListPropertyField(JumpSoundsProp, "Jump Sounds", "Controls the sounds that happens when this AI jumps. \n\n" +
                    "Note: An AI must have a NavMesh Link Component as well as NavMesh Links generated in order for this to work.", true);

                EditorGUILayout.Space();
                EditorGUILayout.Space();

                CustomEditorProperties.CustomFloatSlider(new Rect(), new GUIContent(), LandVolumeProp, "Land Volume", 0, 1);
                CustomEditorProperties.CustomHelpLabelField("Controls the volume of Land Sounds.", true);

                CustomEditorProperties.CustomListPropertyField(LandSoundsProp, "Land Sounds", "Controls the sounds that happens when this AI lands. \n\n" +
                    "Note: An AI must have a NavMesh Link Component as well as NavMesh Links generated in order for this to work.", true);
                CustomEditorProperties.EndFoldoutWindowBox();
            }
        }
