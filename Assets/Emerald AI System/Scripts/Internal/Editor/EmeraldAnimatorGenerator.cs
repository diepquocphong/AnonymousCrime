...
namespace EmeraldAI.Utility
{
    public class EmeraldAnimatorGenerator
    {
        ...
        public static void GenerateAnimatorController(AnimationProfile EmeraldAnimationComponent)
        {
            ...
            NavMeshLinkAnimations(EmeraldAnimationComponent, m_AnimatorController);
        ...
        static void SetState(ChildAnimatorStateMachine childAnimatorStateMachine, string stateName, ChildAnimatorState[] childAnimatorState, List<AnimationClass> animationList)
        {
            if (childAnimatorStateMachine.stateMachine.name == stateName)
            {
                ...
                //Set each state that's greater than the passed list count to null so it is no longer present on the state.
        ...
        static void ApplyType1Animations(AnimationProfile EmeraldAnimationComponent, AnimatorController m_AnimatorController)
        {
            ...
            for (int i = 0; i < m_AnimatorController.layers[0].stateMachine.states.Length; i++)
            {
                ...
                else if (m_AnimatorController.layers[0].stateMachine.states[i].state.name == "Jump")
                {
                    m_AnimatorController.layers[0].stateMachine.states[i].state.motion = EmeraldAnimationComponent.NavMeshLinkAnimations.Jump.AnimationClip;
                    m_AnimatorController.layers[0].stateMachine.states[i].state.speed = EmeraldAnimationComponent.NavMeshLinkAnimations.Jump.AnimationSpeed;
                }
            }
        ...
        /// <summary>
        /// Assign all NavMesh Link Animations (Jump, Land, Drop, and Vault).
        /// </summary>
        static void NavMeshLinkAnimations(AnimationProfile EmeraldAnimationComponent, AnimatorController m_AnimatorController)
        {
            //Go through each sub-state by name and assign the animation to each state within the sub-state using an index
            for (int i = 0; i < m_AnimatorController.layers[0].stateMachine.stateMachines.Length; i++)
            {
                ...
                //NavMehs Link Animations
                AnimationClass[] NavMeshLinkList = new AnimationClass[4] { EmeraldAnimationComponent.NavMeshLinkAnimations.Drop, EmeraldAnimationComponent.NavMeshLinkAnimations.Jump, EmeraldAnimationComponent.NavMeshLinkAnimations.Vault, EmeraldAnimationComponent.NavMeshLinkAnimations.Land };
                SetState(childAnimatorStateMachine, "NavMesh Link", childAnimatorState, NavMeshLinkList.ToList());
            }
        }
