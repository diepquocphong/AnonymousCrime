...
namespace PampelGames.Shared.Utility
{
    public static class PGMeshCreation
    {
        ...
        /// <param name="uvEnd">UV.y end coordinate of the radial faces.</param>
        ...
        /// <summary>
        ///     Creates a cylinder between two specified points.
        /// </summary>
        /// <param name="start">The starting point of the cylinder.</param>
        /// <param name="end">The ending point of the cylinder.</param>
        /// <param name="topRadius">The radius of the cylinder at the top.</param>
        /// <param name="bottomRadius">The radius of the cylinder at the bottom.</param>
        /// <param name="verticalSegments">The number of vertical segments the cylinder should be divided into.</param>
        /// <param name="radialSegments">The number of radial segments the cylinder should be divided into.</param>
        ...
        ///     (y). The angle for top and bottom closing center vertices is -1f.
        /// </param>
        public static void Cylinder(Mesh mesh, Vector3 start, Vector3 end, float topRadius, float bottomRadius,
            int verticalSegments, int radialSegments, bool closed, float uvStart = 0f, float uvEnd = 1f, bool markIndexes = false)
        {
            CylinderFromPointsInternal(mesh, start, end, topRadius, bottomRadius, verticalSegments, radialSegments, closed, uvStart, uvEnd,
                markIndexes);
        }
        ...
        private static void CylinderFromPointsInternal(Mesh mesh, Vector3 start, Vector3 end, float topRadius, float bottomRadius,
            int verticalSegments, int radialSegments, bool closed, float uvStart, float uvEnd, bool markIndexes)
        {
            mesh.Clear();
            ...
            var bitangent = Vector3.Cross(dir, tangent);
            ...
            for (var v = 0; v <= verticalSegments; v++)
            {
                ...
                var t = v / (float) verticalSegments;
                var currentRadius = Mathf.Lerp(topRadius, bottomRadius, t);
                var uvY = Mathf.Lerp(uvStart, uvEnd, t);

                var center = Vector3.Lerp(start, end, t);

                for (var r = 0; r <= radialSegments; r++)
                {
                    var u = r / (float) radialSegments;
                    var angle = u * Mathf.PI * 2.0f;

                    var radialDir = Mathf.Sin(angle) * tangent + Mathf.Cos(angle) * bitangent;
                    var vertex = center + radialDir * currentRadius;

                    vertices.Add(vertex);
                    normals.Add(radialDir);
                    UVs.Add(new Vector2(1 - u, 1 - uvY));
                    row.Add(vertices.Count - 1);

                    if (markIndexes)
                    {
                        var verticalSegmentIndex = verticalSegments - v;
                        var uvAngle = 360f * r / radialSegments;
                        UVs03.Add(new Vector2(verticalSegmentIndex, uvAngle));
                    }
                }
            ...
            for (var v = 0; v < verticalSegments; v++)
            {
                var i1 = vertexRows[v][r];
                var i2 = vertexRows[v + 1][r];
                var i3 = vertexRows[v + 1][r + 1];
                var i4 = vertexRows[v][r + 1];
            ...
            if (closed)
            {
                ...
                if (topRadius > 0f)
                {
                    ...
                    normals.Add(-dir);
                    UVs.Add(new Vector2(0.5f, 0));
                    if (markIndexes) UVs03.Add(new Vector2(verticalSegments, -1f));

                    var centerIndex = vertices.Count - 1;
                    for (var r = 0; r < radialSegments; r++)
                    {
                        var i1 = vertexRows[0][r];
                        var i2 = vertexRows[0][r + 1];
                        triangles.Add(i1);
                        triangles.Add(i2);
                ...
                if (bottomRadius > 0f)
                {
                    ...
                    normals.Add(dir);
                    UVs.Add(new Vector2(0.5f, 1));
                    if (markIndexes) UVs03.Add(new Vector2(0, -1f));

                    var centerIndex = vertices.Count - 1;
                    var vIndex = vertexRows.Count - 1;

                    for (var r = 0; r < radialSegments; r++)
                    {
                        var i1 = vertexRows[vIndex][r + 1];
                        var i2 = vertexRows[vIndex][r];
                        triangles.Add(i1);
                        triangles.Add(i2);
                        triangles.Add(centerIndex);
                    }
