...
namespace PampelGames.Shared.Utility
{
    public static class PGSkinnedMeshUtility 
    {
        ...
        private static bool CombineSkinnedMeshesInternal(GameObject rootObj, List<SkinnedMeshRenderer> renderers, bool saveMesh)
        {
            ...
            {
                var rendererMesh = renderers[i].sharedMesh;
                for (int shapeIndex = 0; shapeIndex < rendererMesh.blendShapeCount; shapeIndex++)
                {
                    string shapeKey = rendererMesh.GetBlendShapeName(shapeIndex);
                    for (int frameIndex = 0; frameIndex < rendererMesh.GetBlendShapeFrameCount(shapeIndex); frameIndex++)
                    {
                        float frameWeight = rendererMesh.GetBlendShapeFrameWeight(shapeIndex, frameIndex);
                        Vector3[] deltaVertices = new Vector3[rendererMesh.vertexCount];
                        Vector3[] deltaNormals = new Vector3[rendererMesh.vertexCount];
                        Vector3[] deltaTangents = new Vector3[rendererMesh.vertexCount];

                        rendererMesh.GetBlendShapeFrameVertices(shapeIndex, frameIndex, deltaVertices, deltaNormals, deltaTangents);

                        Vector3[] finalVertices = new Vector3[mesh.vertexCount];
                        Vector3[] finalNormals = new Vector3[mesh.vertexCount];
                        Vector3[] finalTangents = new Vector3[mesh.vertexCount];

                        Array.Copy(deltaVertices, 0, finalVertices, vertexOffset, deltaVertices.Length);
                        Array.Copy(deltaNormals, 0, finalNormals, vertexOffset, deltaNormals.Length);
                        Array.Copy(deltaTangents, 0, finalTangents, vertexOffset, deltaTangents.Length);

                        var existingIndex = mesh.GetBlendShapeIndex(shapeKey);
                        if(existingIndex >= 0) continue;
                        mesh.AddBlendShapeFrame(shapeKey, frameWeight, finalVertices, finalNormals, finalTangents);
                    }
                }
                vertexOffset += rendererMesh.vertexCount;
            }
            /********************************************************************************************************************************/
        ...
        private static Transform GetMostUpperRootBone(List<SkinnedMeshRenderer> renderers)
        {
            ...
            Transform upperRootBone = allBones.FirstOrDefault(bone => allBones.All(otherBone => otherBone == bone || !IsChildOf(bone, otherBone)));
            ...
            IEnumerable<Transform> GetBonesHierarchy(Transform root)
            {
                ...
                for (var i = 0; i < bones.Count; i++) foreach (Transform child in bones[i]) bones.Add(child);
                return bones;
            }
