...
namespace PampelGames.Shared.Utility
{
    public static class PGMeshAPIUtility
    {
        ...
        /// <summary>
        ///     Sets mesh data using Unity's Advanced Mesh API including <see cref="Color" /> values.
        /// </summary>
        public static void SetBufferData04(Mesh mesh, NativeArray<VertexStruct04> vertexStruct04s, NativeArray<int> triangles)
        {
            mesh.SetVertexBufferParams(vertexStruct04s.Length, PGMeshAPILayout.VertexLayout04);
            mesh.SetVertexBufferData(vertexStruct04s, 0, 0, vertexStruct04s.Length);
            mesh.SetIndexBufferParams(triangles.Length, IndexFormat.UInt32);
            mesh.SetIndexBufferData(triangles, 0, 0, triangles.Length);
        }

        public static void SetBufferData04(Mesh mesh, VertexStruct04[] vertexStruct04s, int[] triangles)
        {
            mesh.SetVertexBufferParams(vertexStruct04s.Length, PGMeshAPILayout.VertexLayout04);
            mesh.SetVertexBufferData(vertexStruct04s, 0, 0, vertexStruct04s.Length);
            mesh.SetIndexBufferParams(triangles.Length, IndexFormat.UInt32);
