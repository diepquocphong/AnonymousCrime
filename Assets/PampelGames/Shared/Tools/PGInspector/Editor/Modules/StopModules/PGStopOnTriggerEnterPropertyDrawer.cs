...
{
    [CustomPropertyDrawer(typeof(PGStopOnTriggerEnter))]
    public class PGStopOnTriggerEnterPropertyDrawer : PropertyDrawer
    {
        private PGStopOnTriggerEnter baseClass;
        ...
        {
            useLayerFilterProperty = property.FindPropertyRelative(nameof(PGStopOnTriggerEnter.useLayerFilter));
            useLayerFilter.BindProperty(useLayerFilterProperty);
            matchingLayersProperty = property.FindPropertyRelative(nameof(PGStopOnTriggerEnter.matchingLayers));
            matchingLayers.BindProperty(matchingLayersProperty);
            useTagFilterProperty = property.FindPropertyRelative(nameof(PGStopOnTriggerEnter.useTagFilter));
            useTagFilter.BindProperty(useTagFilterProperty);
            matchingTagsProperty = property.FindPropertyRelative(nameof(PGStopOnTriggerEnter.matchingTags));

            var obj = fieldInfo.GetValue(property.serializedObject.targetObject);
            var objList = obj as List<PGStopClassBase>;
            var index = PGInspectorEditorUtility.GetDrawingListIndex(property);
            if (objList != null) baseClass = objList[index] as PGStopOnTriggerEnter;
