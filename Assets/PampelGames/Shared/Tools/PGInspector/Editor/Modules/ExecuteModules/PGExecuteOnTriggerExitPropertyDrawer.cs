...
{
    [CustomPropertyDrawer(typeof(PGExecuteOnTriggerExit))]
    public class PGExecuteOnTriggerExitPropertyDrawer : PropertyDrawer
    {
        private PGExecuteOnTriggerExit baseClass;
        ...
        {
            useLayerFilterProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerExit.useLayerFilter));
            useLayerFilter.BindProperty(useLayerFilterProperty);
            matchingLayersProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerExit.matchingLayers));
            matchingLayers.BindProperty(matchingLayersProperty);
            useTagFilterProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerExit.useTagFilter));
            useTagFilter.BindProperty(useTagFilterProperty);
            matchingTagsProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerExit.matchingTags));

            var obj = fieldInfo.GetValue(property.serializedObject.targetObject);
            var objList = obj as List<PGExecuteClassBase>;
            var index = PGInspectorEditorUtility.GetDrawingListIndex(property);
            if (objList != null) baseClass = objList[index] as PGExecuteOnTriggerExit;
