...
{
    [CustomPropertyDrawer(typeof(PGExecuteOnTriggerEnter))]
    public class PGExecuteOnTriggerEnterPropertyDrawer : PropertyDrawer
    {
        private PGExecuteOnTriggerEnter baseClass;
        ...
        {
            useLayerFilterProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerEnter.useLayerFilter));
            useLayerFilter.BindProperty(useLayerFilterProperty);
            matchingLayersProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerEnter.matchingLayers));
            matchingLayers.BindProperty(matchingLayersProperty);
            useTagFilterProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerEnter.useTagFilter));
            useTagFilter.BindProperty(useTagFilterProperty);
            matchingTagsProperty = property.FindPropertyRelative(nameof(PGExecuteOnTriggerEnter.matchingTags));

            var obj = fieldInfo.GetValue(property.serializedObject.targetObject);
            var objList = obj as List<PGExecuteClassBase>;
            var index = PGInspectorEditorUtility.GetDrawingListIndex(property);
            if (objList != null) baseClass = objList[index] as PGExecuteOnTriggerEnter;
