...
{
    /// <summary>
    ///     Static helper methods for MonoBehaviour Inspectors that implement <see cref="PGExecuteClassBase" />.
    /// </summary>
    public static class PGExecuteClassInspectorUtility
    {
            ...
            List<string> executeClassBaseStringList)
        {
            ...
            foreach (var executeClassBase in executeClassBaseList) executeClassBaseStringList.Add(executeClassBase.ModuleName());
        }
            ...
            addExecuteDropdown.RegisterCallback<ChangeEvent<string>>(evt =>
            {
                if (addExecuteDropdown.index >= 0)
                {
                    ...
                    foreach (var executeClassBase in baseComponentExecuteClassBaseList)
                        if (executeClassBaseList[addExecuteDropdown.index].ModuleName() == executeClassBase.ModuleName())
                        {
                            canAdd = false;
                            EditorUtility.DisplayDialog("Invalid Item", executeClassBaseList[addExecuteDropdown.index].ModuleName() +
            ...
            DropdownField addExecuteDropdown, List<PGExecuteClassBase> executeClassBaseList, VisualElement visualElementParent, int parentInsertIndex)
        {
            ...
            void BindItem(VisualElement itemWrapper, int i)
            {
                ...
                itemLabel.text = "Execute: " + baseComponentExecuteClassBaseList[i].ModuleName();
                itemLabel.tooltip = baseComponentExecuteClassBaseList[i].ModuleInfo();
