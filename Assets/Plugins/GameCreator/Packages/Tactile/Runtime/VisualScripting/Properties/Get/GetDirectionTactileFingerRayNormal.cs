...
{
    [Title("Finger Ray Normal")]
    [Category("Tactile/Finger Ray Normal")]
    
    [Description(
        "Gets the raycast hit normal at finger position of the Tactile Control in world space"
    )]
    ...
    [Serializable]
    public class GetDirectionTactileFingerRayNormal : PropertyTypeGetDirection
    {
        // MEMBERS: -------------------------------------------------------------------------------
        ...
        public override string String => 
            $"{this.m_TactileControl}.Finger[{this.m_FingerIndex}] Normal";
        ...
        public override Vector3 Get(Args args)
        {
            ...
            if (control == null) return Vector3.up;
            ...
            if (finger == null) return Vector3.up;

            var maxDistance = (float)this.m_MaxDistance.Get(args);
            var planeTransform = this.m_Plane.Get<Transform>(args);

            return this.GetRayNormal(camera, planeTransform, finger.screenPosition, maxDistance);
        }
        ...
        public static PropertyGetDirection Create => new (new GetDirectionTactileFingerRayNormal());
