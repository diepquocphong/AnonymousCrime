...
namespace NinjutsuGames.StateMachine.Runtime
{
    ...
    public abstract class BaseNode
    {
        ...
        [NonSerialized] public GameObject _context;

        [field:NonSerialized] protected List<int> IsContextRunning { get; } = new();
        [field:NonSerialized] protected List<int> IsContextDisabled { get; } = new();
        
        protected int NodeId(GameObject nodeContext) => nodeContext.GetInstanceID() + GUID.GetHashCode();
        ...
        private List<BaseNode> cachedOutputNodes;
        private List<BaseNode> cachedInputNodes;
        private bool isOutputNodesCacheValid;
        private bool isInputNodesCacheValid;
        ...
        private struct PortUpdate : IEquatable<PortUpdate>
        {
            ...
            {
                return Equals(fieldNames, other.fieldNames) && Equals(node, other.node);
            }

            public override bool Equals(object obj)
            {
                return obj is PortUpdate other && Equals(other);
            }

            public override int GetHashCode()
            {
                return HashCode.Combine(fieldNames, node);
            }
        }

        // Used in port update algorithm
        private readonly Stack<PortUpdate> fieldsToUpdate = new();
        private readonly HashSet<PortUpdate> updatedFields = new();
        ...
        {
            return CreateFromType(typeof(T), new Rect(position, new Vector2(100, 100))) as T;
        }
        
        /// <summary>
        /// Creates a node of type T at a certain position and size
        /// </summary>
        /// <param name="rect"></param>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public static T CreateFromType<T>(Rect rect) where T : BaseNode
        {
            return CreateFromType(typeof(T), rect) as T;
        }
        ...
        {
            return CreateFromType(nodeType, new Rect(position, new Vector2(100, 100)));
        }

        /// <summary>
        /// Creates a node of type nodeType with a rect
        /// </summary>
        /// <param name="position">position in the graph in pixels</param>
        /// <typeparam name="nodeType">type of the node</typeparam>
        /// <returns>the node instance</returns>
        public static BaseNode CreateFromType(Type nodeType, Rect rect)
        {
        ...
        {
            if (this is not BaseGameCreatorNode { useNetwork: true } gameCreatorNode) return;
            gameCreatorNode.networkingSettings ??= new NetworkingSettings(gameCreatorNode.GUID);
            gameCreatorNode.networkingSettings.nodeId = gameCreatorNode.GUID;
            graph.CheckNetworkNode(gameCreatorNode.networkingSettings.networkSync, gameCreatorNode.GUID);
        }
        ...
        // public bool CanExecute(GameObject context) => !context ? enabledForExecution : !IsContextDisabled.Contains(NodeId(context));
        public bool CanExecute(GameObject currentContext) => enabledForExecution && 
                                                      (!currentContext || (!IsContextDisabled.Contains(NodeId(currentContext)) && currentContext.activeInHierarchy && currentContext.Get<StateMachineRunner>().enabled));
        ...
        {
            isOutputNodesCacheValid = false;
            isInputNodesCacheValid = false;
        }
        
        public void InvalidateOutputNodesCache()
        {
            isOutputNodesCacheValid = false;
        }
        
        public void InvalidateInputNodesCache()
        {
            isInputNodesCacheValid = false;
        }
        ...
        public bool UpdatePortsForFieldLocal(string fieldName, bool sendPortUpdatedEvent = true)
        {
            ...
            if (!nodeFields.TryGetValue(fieldName, out var fieldInfo))
                return false;
        ...
        void InitializeInOutDatas()
        {
            ...
            foreach (var method in methods)
            {
                ...
                if (nodeFields.TryGetValue(customPortBehaviorAttribute.fieldName, out var field))
        ...
        public void OnEdgeConnected(SerializableEdge edge)
        {
            ...
            var portCollection = (input) ? (NodePortContainer)inputPorts : outputPorts;

            portCollection.Add(edge);
            if (input)
            {
                InvalidateInputNodesCache();
            }
            else
            {
                InvalidateOutputNodesCache();
            }
            UpdateAllPorts();
        ...
        public void OnProcess(GameObject context, Args customArgs)
        {
        ...
        /// </summary>
        protected virtual void Process(GameObject newContext, Args customArgs = null)
        {
        }
        ...
        {
            if (isInputNodesCacheValid && cachedInputNodes != null) return cachedInputNodes;
            cachedInputNodes = new List<BaseNode>();
            foreach (var edge in inputPorts.SelectMany(port => port.GetEdges()))
            {
                cachedInputNodes.Add(edge.outputNode);
            }
            isInputNodesCacheValid = true;
        ...
        {
            if (isOutputNodesCacheValid && cachedOutputNodes != null) return cachedOutputNodes;
            cachedOutputNodes = new List<BaseNode>();
            foreach (var edge in outputPorts.SelectMany(port => port.GetEdges()))
            {
                cachedOutputNodes.Add(edge.inputNode);
            }
            isOutputNodesCacheValid = true;
            return cachedOutputNodes;
        }
        ...
        {
            return GetAllPorts().SelectMany(port => port.GetEdges());
        }
