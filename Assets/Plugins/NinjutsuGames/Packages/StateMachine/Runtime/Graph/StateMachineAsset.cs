...
namespace NinjutsuGames.StateMachine.Runtime
{
    ...
    public class StateMachineAsset : TGlobalVariables, INameVariable, ISerializationCallbackReceiver
    {
        ...
        public void RunNode(string nodeId, Args args)
        {
            ...
            nodesPerGUID[nodeId].OnProcess(args.Self, args);
        }
        ...
        public static void SyncVariables(NameList from, NameList to)
        {
            ...
            var runnerVars = from;
            var runnerVarNames = new List<string>(runnerVars.Names);
        
            var assetVars = to;
            var assetVarNames = new List<string>(assetVars.Names);

            for (var i = 0; i < runnerVars.Length; i++)
            {
                var id = runnerVars.Names[i];
                if(assetVarNames.Contains(id)) continue;

                var copy = runnerVars.Get(i).Copy as NameVariable;
                assetVars.Add(copy);
            }
        
            for (var i = 0; i < assetVars.Length; i++)
            {
                var id = assetVars.Names[i];
                if (runnerVarNames.Contains(id)) continue;
            ...
            {
                var id = runnerVars.Names[i];
                var hasIt = assetVarNames.Contains(id);
                if(i >= assetVars.Length) continue;
                if (hasIt && assetVars.Get(i).TypeID.Hash != runnerVars.Get(i).TypeID.Hash)
                {
                    assetVars.Remove(i);
